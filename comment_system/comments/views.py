...
from .serializers import UserSerializer, CommentSerializer, CourseSerializer, CourseDetailSerializer, RatingSerializer
...
class CommentView(APIView):
    ...
    def get(self, request, course_id):
        # Fetch comments related to a specific course
        try:
    ...
    def post(self, request, course_id):
        # Add a comment to a specific course
        try:
            course = Course.objects.get(id=course_id)
...
class CourseDetailView(APIView):
    ...
    def post(self, request, course_id):
        try:
            ...
            # Save the rating
            rating = Rating.objects.create(course=course, user=request.user, rating=rating_value)
...
class QuizzDeleteView(APIView):
    def delete(self, request, pk):
        try:
            ...
            return Response({"error": "Quizz not found"}, status=status.HTTP_404_NOT_FOUND)
